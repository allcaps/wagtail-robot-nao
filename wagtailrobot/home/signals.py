...
def say_hello(sender, user, request, **kwargs):
    ...
    conn.playAsync("animations/Sit/Emotions/Neutral/AskForAttention_1")
    conn.asyncTakePicturePNG(name)
...
def give_comment_on_page_title(sender, instance, revision, **kwargs):
    ...
    if _("page") in title or _("title") in title:
        adjective = _("stupid")
        conn.playAsync('animations/Stand/Emotions/Negative/Angry_1')
